...
namespace CMSC312_Project
{
    class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            ...
            List<Process> NewList2 = new List<Process>();
            List<Process> ReadyList = new List<Process>();
            List<Process> WaitingList = new List<Process>();
            List<Process> WaitingList2 = new List<Process>();
            ...
            //run SJF scheduler
            s1.Start();
            SJF(SortedReadyList, NewList, WaitingList, FullList, countId + 1, m);
            s1.Stop();

            if (SortedReadyList.Count == 0 && NewList.Count == 0 && WaitingList.Count == 0)
            {
                Console.WriteLine("All processors processes(SJF)");
            }

            //run RR scheduler
            s2.Start();
            RoundRobin(ReadyList, NewList2, WaitingList2, countId + 1, m);
            s2.Stop();

            if (ReadyList.Count == 0 && NewList2.Count == 0 && WaitingList2.Count == 0)
            {
                Console.WriteLine("All processors processes(RR)");
            }

            Console.WriteLine("Elapsed Time for SJF is {0} ms", s1.ElapsedMilliseconds);
            Console.WriteLine("Elapsed Time for RR is {0} ms", s2.ElapsedMilliseconds);
        ...
        //***************************************************************************
        public static bool calculate(Process pr, List<Process> waitingList, List<Process> readyList, int loops)
        {
            Random random = new Random();
            ...
            while ((loops > 0) && (pr.Steps[pr.location].Loops > 0))
            {
            ...
            if (pr.Steps[pr.location].Loops == 0)
            {
            ...
            {
                Console.WriteLine("Process {0} stopped at {1}", pr.id, pr.Steps[pr.location].Loops);
                pr.State = "READY";
                readyList.Add(pr);
                Console.WriteLine("Process {0} is now: {1}", pr.id, pr.State);
        ...
        public static void SJFThread(Process pr, List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, List<Process> FullList, int countId, MemoryManager m, Semaphore semaphore)
        {
            ...
            {
                terminate(pr, m, ReadyList, WaitingList, NewList);
            }
            ...
            while (pr.location < pr.Steps.Count)
            {
                ...
                {
                    bool calculated = calculate(pr, WaitingList, ReadyList, pr.Steps[pr.location].Loops);
                    if (!calculated)
                    {
                        break;
                    }
                ...
                {
                    fork(pr, countId, ReadyList, WaitingList, NewList, m);
                }
        ...
        //input: Process pr, MemoryManager m, List<Process> ReadyList, List<Process> WaitingList, List<Process> NewList
        //output: void
        //***************************************************************************************************************
        //This method terminates the process and also terminates the children of the process if they have still not terminated.
        //***************************************************************************************************************
        ...
        public static void terminate(Process pr, MemoryManager m, List<Process> ReadyList, List<Process> WaitingList, List<Process> NewList )
        {
            ...
            ReadyList.Remove(pr);
            WaitingList.Remove(pr);
            NewList.Remove(pr);

            //if terminated child has a parent
            ...
            //if the process has children still running despite terminating
            //    terminate all children of process (single level only)
            if (pr.children.Count > 0)
            {
                int i = 0;
                while(pr.children.Count > 0)
                {
                    terminate(pr.children[i], m, ReadyList, WaitingList, NewList);
                    i++;
                }
            }
        }
        ...
        public static void SJF(List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, List<Process> FullList, int countId, MemoryManager m)
        {
            ...
            while (ReadyList.Count != 0 || WaitingList.Count != 0 || NewList.Count != 0 || thr1.IsAlive)
            {
                ...
                //at the end of each loop increase the priority of each process
                foreach(var p in ReadyList)
                {
                    p.priority++;
                }
        ...
        //***************************************************************************************************************************************
        // RRThread
        // input:Process pr, List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, int countId, MemoryManager m, Semaphore s
        // output: void
        //****************************************************************************************************************************************
        //Thread method that runs through each process in Round Robin manner.
        //****************************************************************************************************************************************
        public static void RRThread(Process pr, List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, int countId, MemoryManager m, Semaphore s )
        {
            ...
            while (pr.location < pr.Steps.Count)
            {
                ...
                if (pr.Steps[pr.location].Command == "CS_START")
                {
                    ...
                    //check to make sure none of the processes have inCS as true
                    if (s.WaitOne(TimeSpan.FromSeconds(4)))
                    {
                        bool goCS = true;
                        foreach (var p in ReadyList)
                        {
                ...
                {
                    bool calculated = calculate(pr, WaitingList, ReadyList, 3);
                    if (!calculated)
                    {
                        break;
                    }
        ...
        //********************************************************************************************************************
        //RoundRobin
        //input: List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, int countId, MemoryManager m
        //output: void
        //********************************************************************************************************************
        //Scheduler method that follows round robin algorithm for comparison against SJF algorithm.
        //********************************************************************************************************************
        public static void RoundRobin(List<Process> ReadyList, List<Process> NewList, List<Process> WaitingList, int countId, MemoryManager m)
        {
            ...
            Thread thr1 = new Thread(() => RRThread(pr1, ReadyList, NewList, WaitingList, countId, m, s));
            ...
            while (ReadyList.Count != 0 || WaitingList.Count != 0 || NewList.Count != 0 || thr1.IsAlive)
            {
                ...
                if (ReadyList.Count > 0 && !thr1.IsAlive)
                {
                    ...
                    if (!thr1.IsAlive)
                    {
                        thr1 = new Thread(() => RRThread(pr0, ReadyList, NewList, WaitingList, countId, m, s));
                        thr1.Start();
                    }
        ...
        public static void fork(Process pr, int countId, List<Process> ReadyList, List<Process> WaitList, List<Process> NewList, MemoryManager m)
        {

            //Make Child Process
            Console.WriteLine("Fork Child");
            //copy the process
            Process newPr = pr.Copy(pr);
            newPr.id = countId;
            countId = countId + 1;

            newPr.Steps.RemoveRange(0, pr.location+1);
            newPr.location = 0;
            ...
            pr.totalLoops = pr.totalLoops + 100;
    ...
    //  -int priority - the higher the number the closer it should be to the front
    //**********************************************************************************
    //  Methods
    ...
    public class Process
    {
        ...
        public Process Copy(Process pr)
        {
            return Clone<Process>(pr);
        }
